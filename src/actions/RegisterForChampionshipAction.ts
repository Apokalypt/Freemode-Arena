...
import {
    ...
    CHAMPIONSHIP_ROLE_ID, EMOJI_FAQ, EMOJI_GREEN_CHECK, EMOJI_MATCHMAKING, EMOJI_RIGHT_ARROW, EMOJI_SUPPORT,
    FAQ_CHANNEL_ID,
    SUPPORT_CHANNEL_ID,
    SUPPORT_ROLE_ID
} from "@constants";
    ...
    protected async _execute(this: RegisterForChampionshipActionExecutionContext<true>): Promise<void> {
        ...
        if (userPlatforms.length === 1) {
            ...
            platforms = await this._askForStringSelection({
                title: "Sur quelle(s) plateforme(s) souhaitez-vous jouer?",
                description: "Choisissez une ou plusieurs plateformes sur lesquelles vous souhaitez jouer ce championnat.",
        ...
        const raw = { _id: user.id, platforms, level: MatchmakingService.instance.getUserLevel(user.id), displayName };
        ...
        await Promise.all([
            thread.send({
                content: `# Bonne chance ${user}\n` +
                    "** **\n" +
                    `${EMOJI_GREEN_CHECK} Vous êtes maintenant inscrit à \`\`Freemode Arena - Saison 5\`\`\n`+
                    "\n" +
